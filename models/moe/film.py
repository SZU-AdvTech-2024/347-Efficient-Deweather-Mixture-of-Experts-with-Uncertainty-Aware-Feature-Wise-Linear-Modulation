...
from .basic_module import KeepTopK, DynamicTopK


####################################  FiLM Modulation MoE  ###############################

class Modular_Experts(nn.Module):
    # generate alpha and beta according to input 'x', and then return output which equals (alpha * x + beta)
...
class Transformer_MoE_FiLM(nn.Module):
    # rather than using deepcopy, may be write independent classes for router and adaptor better?
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0., num_expert=0,
        ...
        # TopK_Function = DynamicTopK(top_k=int(top_k))
        ##################################################################
        ...
        for _ in range(depth):
            # [attention, router, adaptor, [laynorm, basenet, act, expert1, expert2, ...]]
    ...
    def forward(self, x, *args, **kwargs):
        ...
        for i, (attn, router, adaptor, ff_list) in enumerate(self.layers):
            ...
            for idx in range(self.num_expert):
                ...
                y += weight_idx * ff_list[idx+3](y_temp)
